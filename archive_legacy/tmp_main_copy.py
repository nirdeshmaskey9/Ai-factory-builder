...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        pass
    logging.getLogger(__name__).info("✅ AI Factory v1.2-master-stable — Core Systems Verified.")
    print(f"✅ AI Factory v1.2-master-stable — Core Systems Verified. (Startup @ {ts})")
...
app = FastAPI(
    ...
    version="1.2-master-stable",
    description="v1.2-master-stable: Hardened core (builders, evaluator, memory, health).",
    lifespan=lifespan,
)
...
def factory_info():
    ...
    try:
        ...
        version = version_path.read_text(encoding="utf-8").strip() if version_path.exists() else "1.2-master-stable"
    except Exception:
        version = "1.2-master-stable"
    ts = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
    mcp = {"planner": True, "builder": True, "evaluator": True, "memory": True}
    healthy = True
    return {"version": version, "timestamp": ts, "running_deployments": running, "healthy": healthy, "mcp": mcp}
...
async def run_self_diagnostics():
    try:
        ...
        if health.get("status") != "healthy":
            print("⚠️ Warning: Startup health degraded:", health.get("summary"))
        else:
